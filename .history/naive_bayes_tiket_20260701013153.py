...
class NaiveBayesClassifier:
    ...
    for kategori, jumlah_dok_kat in self.category_count.items():
        ...
        for word in words:
            ...
            likelihood = (
                (word_freq + 1)
                / (self.total_words_per_category[kategori] + vocab_size)
            )
...
if __name__ == "__main__":
    ...
    for test in testing_data:
        ...
        print(f"Label Asli: {test['kategori']}")
        print(
            f"Prediksi : {prediksi['kategori']} "
            f"(Prioritas: {prediksi['prioritas']})"
        )
        print(f"Confidence : {prediksi['score']}%")
        print(status)

        print("Score tiap kategori:")
